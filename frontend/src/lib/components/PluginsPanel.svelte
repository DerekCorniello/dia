      ...
      {#each plugins as p (p.id + p.source)}
        <li class="rounded border border-bg-600 bg-bg-800 p-3 text-sm">
          ...
          {#if p.longDescription}
            <details class="mt-1">
              <summary class="cursor-pointer text-[10px] text-primary hover:text-primary/80">more details</summary>
              <p class="mt-1 whitespace-pre-wrap text-xs text-fg-dim">{p.longDescription}</p>
            </details>
          {/if}
          <div class="mt-1 flex flex-wrap items-center gap-2">
            <span class="inline-flex rounded-full px-1.5 py-0.5 text-[9px] font-medium {statusClass(p.status)}">{p.status}</span>
            {#if p.lastError}
              <span class="inline-flex shrink-0 items-center gap-1 rounded-full bg-error/10 px-1.5 py-0.5 text-[9px] text-error" title={p.lastError}>
                error
              </span>
            {/if}
            <button
